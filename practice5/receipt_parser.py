...
# ^ and $ ensure NOTHING else is on that line
pattern = r"^\d+(?:\s\d+)?,\d{2}$"
# Find all matches using MULTILINE mode
raw_prices = re.findall(pattern, text, re.MULTILINE)
clean_prices = list(dict.fromkeys(raw_prices))
...
data = {
    ...
    "all_detected_prices": clean_prices,
