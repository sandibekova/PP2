...
pattern2 = r"ab{2,3}"
print(f"Task 2 (2 'b's): {bool(re.fullmatch(pattern2, 'ab'))}")
...
pattern5 = r"a.*b"
print(f"Task 5 Result: {bool(re.fullmatch(pattern5, 'axxxbc'))}")
...
def snake_to_camel(text):
    # Pattern: finds an underscore followed by a letter
    # Logic: replaces the match with the uppercase version of that letter
    return re.sub(r'_([a-z])', lambda x: x.group(1).upper(), text)

# Usage
result = snake_to_camel("hello_world_practice")
